...
assert.equal(snapshot.length, 35, `A projektben ${snapshot.length} ETO-rekord található 35 helyett.`);
fs.writeFileSync(new URL('../eto-discovery.json', import.meta.url), `${JSON.stringify(snapshot, null, 2)}\n`);
console.log('✓ ETO FC pontos 35-ös projektlista artifactként elkészült');
