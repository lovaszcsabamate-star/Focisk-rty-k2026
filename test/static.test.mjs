...
const appConfiguration = read('../js/app/configuration.js');
const saveServiceModule = read('../js/services/save-service.js');
const seasonSaveServiceModule = read('../js/services/season-save-service.js');
const pwaCss = read('../css/pwa.css');
const pwaJs = read('../js/pwa.js');
const bootstrap = read('../js/bootstrap.js');
const seasonModelModule = read('../js/database/season-model.js');
const seasonServiceModule = read('../js/database/season-service.js');
...
assert.match(mobileExperienceJs, /APP_STORAGE_KEYS\.savedMatch/);
assert.match(saveServiceModule, /SAVED_MATCH_VERSION/);
assert.match(seasonSaveServiceModule, /createSavedMatchSnapshot/);
assert.match(seasonSaveServiceModule, /savedMatchMatchesSeason/);
assert.match(mobileExperienceJs, /services\/season-save-service\.js/);
assert.match(appConfiguration, /savedMatch:\s*'fociskartyak:saved-match:v2'/);
assert.match(mobileExperienceJs, /hydrateSeasonGame/);
...
assert.match(seasonModelModule, /createSeasonCardId/);
assert.match(seasonServiceModule, /loadSeason/);
assert.match(databaseRegistryModule, /loadDatabaseRegistry/);
assert.match(databaseRegistryModule, /validateDatabaseManifest/);
assert.match(databaseRegistryModule, /getAvailableSeasons/);
...
assert.match(buildScript, /season-save-service\.js/);
assert.match(serviceWorker, /js\/database\/database-service\.js/);
assert.match(serviceWorker, /js\/database\/season-service\.js/);
...
assert.equal(databaseRegistry.schemaVersion, 2);
assert.equal(databaseRegistry.defaultDatabaseId, 'hungary-nb1-2025-26');
assert.equal(databaseRegistry.defaultSeasonId, '2025-26');
assert.equal(databaseEntry.enabled, true);
assert.equal(databaseEntry.competitionId, 'hungary-nb1');
assert.equal(databaseEntry.seasonId, '2025-26');
assert.equal(databaseManifest.id, databaseEntry.id);
assert.equal(databaseManifest.schemaVersion, 2);
...
assert.match(serviceWorker, /const CORE_SHELL/);
...
assert.match(main, /readSeasonSavedMatch/);
assert.match(main, /writeSeasonSavedMatch/);
...
assert.match(standalone, /globalThis\.__FOCISKARTYAK_SEASON__/);
...
assert.match(standalone, /"seasonId":"2025-26"/);
assert.doesNotMatch(standalone, /<script type="module" src=/);
assert.doesNotMatch(standalone, /<link rel="stylesheet" href=/);

console.log('✓ Magyar, reszponzív, szezonhoz kötötten menthető és több idényre bővíthető offline felületi szerződés: rendben');
