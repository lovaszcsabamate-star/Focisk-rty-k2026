...
const makePlayers = (count, { club, nation, offset = 0 }) => Array.from({ length: count }, (_, index) => ({
  ...
  nation,
  stats: { goals: 0 },
  meta: { clubId: canonicalClubKey(club).replace(/\s+/g, '-') },
}));
...
assert.equal(MIN_NATIONAL_DECK_SIZE, 8);
...
const romanianAvailable = validateDeckSelection(players, { kind: 'nation', value: 'Román' });
assert.equal(romanianAvailable.valid, true);
assert.equal(romanianAvailable.players.length, 10);
...
const quickCatalog = buildQuickMatchCatalog(players);
assert.equal(quickCatalog[QUICK_MATCH_CATEGORY.NATIONAL].some(item => item.key === 'romania'), true);
assert.equal(quickCatalog[QUICK_MATCH_CATEGORY.NATIONAL].find(item => item.key === 'romania')?.minimum, 8);
assert.equal(
  quickMatchSelectionsCompatible(
    { kind: 'nation', value: 'Hungary' },
    { kind: 'federation', value: 'UEFA' },
  ),
  false,
);
assert.equal(
  quickMatchSelectionsCompatible(
    { kind: 'nation', value: 'Hungary' },
    { kind: 'nation', value: 'Serbia' },
  ),
  true,
);
const hungaryEntry = quickCatalog[QUICK_MATCH_CATEGORY.NATIONAL].find(item => item.key === 'hungary');
assert.equal(quickMatchOpponentEntries(quickCatalog, hungaryEntry).every(item => item.kind === 'nation'), true);
const forbiddenPairing = validateQuickMatchPairing(
  players,
  { kind: 'nation', value: 'Hungary' },
  { kind: 'federation', value: 'UEFA' },
);
assert.equal(forbiddenPairing.valid, false);
assert.equal(forbiddenPairing.code, 'INCOMPATIBLE_OPPONENT');
...
const quickMatchDomainSource = read('../js/domain/quick-match-domain.js');
const cardComponentSource = read('../js/ui/card-component.js');
const scoreboardSource = read('../js/ui/scoreboard-component.js');
...
console.log('✓ 8 lapos ligaválogatott, azonos típusú párosítás és minden módú logómegjelenítés: rendben');
