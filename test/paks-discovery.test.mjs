...
assert.equal(snapshot.length, 33, `A projektben ${snapshot.length} Paks-rekord található 33 helyett.`);
fs.writeFileSync(new URL('../paks-discovery.json', import.meta.url), `${JSON.stringify(snapshot, null, 2)}\n`);
console.log('✓ Paks pontos 33-as projektlista artifactként elkészült');
