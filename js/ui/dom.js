/** Közös DOM- és assetsegédek a vizuális komponensekhez. */
...
export const initials = name => name.split(' ').filter(Boolean).map(word => word[0]).join('').slice(0, 2).toUpperCase();
export const finiteDetail = value => typeof value === 'number' && Number.isFinite(value) ? String(value) : null;
